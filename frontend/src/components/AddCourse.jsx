...
const AddCourse = () => {
  ...
  const handleChange = (e) => {
    ...
    setCourseData({
      ...courseData,
      [name]: name === 'courseThumbnail' ? files[0] : value,
    });
  };
  ...
  <form onSubmit={handleSubmit} className="max-w-lg mx-auto">
    ...
    <div className="mb-4">
      ...
      <input type="text" name="status" id="status" value={courseData.status} onChange={handleChange} className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-lg border-gray-300 rounded-md" />
    </div>
    <br/>
    <div className="flex justify-between">
      <Link to={`/display`} className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">View Details</Link>
